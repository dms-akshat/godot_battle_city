...
signal press_shoot2(direction, pos)
var SPEED = 350
...
func _physics_process(delta):
	if not Globals.is_game_over:
		...
		if(Input.is_action_just_released("p2_up") || Input.is_action_just_released("p2_down") || Input.is_action_just_released("p2_left") || Input.is_action_just_released("p2_right")):
			velocity = Vector2(0,0)
		
		if(Input.is_action_pressed("p2_fire") and can_shoot):
			#print('fire')
			can_shoot=false
			$ShootTimer.start()
			if Globals.score>0:
			...
			press_shoot2.emit(direction,pos)
